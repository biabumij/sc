...
class Pmm_model extends CI_Model {
   ...
   function GetSPOByWeek($schedule_id,$week)
   {
	...
	function GetReceiptMat13($supplier_id=false,$purchase_order_no=false,$start_date=false,$end_date=false,$filter_material=false)
    {
        ...
        $this->db->select('ppp.id, ppp.tanggal_invoice, ppp.nomor_invoice, ppp.memo, SUM(ppd.qty * price) + SUM(ppd.tax) as tagihan,(select sum(total) from pmm_pembayaran ppm where ppm.penagihan_id = ppp.id and status = "DISETUJUI") as pembayaran, round((SUM(ppd.qty * price) + SUM(ppd.tax)) - (select sum(total) from pmm_pembayaran ppm where ppm.penagihan_id = ppp.id and status = "DISETUJUI"),0) as piutang');
